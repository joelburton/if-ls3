...
static void print_preamble(void)
{
    ...
    switch(error_format)
    {
        case 0:  /* RISC OS error message format */
            ...
            if (!(ErrorReport.main_flag)) fprintf(stderr, "\"%s\", ", p);
            fprintf(stderr, "line %d: ", ErrorReport.line_number);
            ...
            if (ErrorReport.orig_file) {
                ...
                if (ErrorReport.orig_line) {
                    fprintf(stderr, ", %d", ErrorReport.orig_line);
                    if (ErrorReport.orig_char) {
                        fprintf(stderr, ":%d", ErrorReport.orig_char);
                    }
                }
                fprintf(stderr, "): ");
            }
        ...
        case 1:  /* Microsoft error message format */
            ...
            fprintf(stderr, "%s", p);
            if (with_extension_flag) fprintf(stderr, "%s", Source_Extension);
            fprintf(stderr, "(%d)", ErrorReport.line_number);
        ...
        case 2:  /* Macintosh Programmer's Workshop error message format */
            ...
            fprintf(stderr, "File \"%s\"; Line %d", p, ErrorReport.line_number);
            ...
            if (ErrorReport.orig_file) {
                ...
                fprintf(stderr, ": (\"%s\"", op);
                if (ErrorReport.orig_line) {
                    fprintf(stderr, "; Line %d", ErrorReport.orig_line);
                    if (ErrorReport.orig_char) {
                        fprintf(stderr, "; Char %d", ErrorReport.orig_char);
                    }
                }
                fprintf(stderr, ")");
            }
...
{
    if (hash_printed_since_newline) fprintf(stderr, "\n");
    hash_printed_since_newline = FALSE;
    print_preamble();
    switch(style)
    {   case 1: fprintf(stderr, "Error: "); no_errors++; break;
        case 2: fprintf(stderr, "Warning: "); no_warnings++; break;
        case 3: fprintf(stderr, "Error:  [linking]  "); no_errors++; break;
        case 4: fprintf(stderr, "*** Compiler error: ");
