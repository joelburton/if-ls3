 ...
 */
function useRangesForSym(index: CompilerIndex, sym: string): Array<{ uri: string; range: Range }> {
  ...
  for (const ref of index.references ?? []) {
    ...
    for (const locStr of ref.locs) {
      ...
      results.push({
        ...
        range: Range.create({ line: line - 1, character: col }, { line: line - 1, character: col + sym.length }),
      });
    }
  }
...
function definitionRange(
  ...
  return Range.create({ line: line - 1, character: col }, { line: line - 1, character: col + name.length });
}
...
function companion(index: CompilerIndex, sym: string, type: string): Companion | null {
  if (type === "action") {
    ...
    if (!index.routines.some((r) => r.name.toLowerCase() === companionName.toLowerCase())) return null;
...
export function affectedFiles(edit: WorkspaceEdit): Set<string> {
  ...
  for (const uri of Object.keys(edit.changes ?? {})) files.add(URI.parse(uri).fsPath);
  return files;
}

export function hasInactiveBranches(index: CompilerIndex, files: Set<string>): boolean {
  return (index.conditionals ?? []).some((c) => files.has(c.file) && inactiveLineRange(c) !== null);
}
...
export function prepareRename(
  ...
  const ref = fileIndex >= 0 ? refAtPosition(index, fileIndex, line1, position.character) : undefined;

  const symName = ref?.sym ?? hit.word;

  // Reject system symbols.
  const symEntry = index.symbols.find((s) => s.name.toLowerCase() === symName.toLowerCase());
  ...
  return {
    range: Range.create({ line: position.line, character: hit.start }, { line: position.line, character: hit.end }),
...
export function computeRename(
  ...
  function addEditsForSymbol(name: string, replaceName: string, isAction: boolean): void {
    // Use-sites.
    for (const { uri, range } of useRangesForSym(index, name)) allEdits.push({ uri, range, newText: replaceName });

    // Definition site.
    if (isAction) {
      // Action symbols live in symbols[] as "Foozle__A".
      const actionSym = index.symbols.find((s) => s.name.toLowerCase() === name + "__a");
  ...
  if (comp) {
    ...
    if (compNewName !== null) addEditsForSymbol(comp.sym, compNewName, !isAction && type === "routine");
  }
