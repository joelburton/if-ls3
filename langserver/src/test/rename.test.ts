...
const renameIndex: CompilerIndex = {
  ...
  objects: [
    {
      name: "TheRoom", file: FILE, start_line: 8, end_line: 10,
  ...
  symbols: [
    ...
    { name: "Foozle__A", type: "fake_action", value: 1, flags: 4, is_system: false,
      file: FILE, line: 15 },
    // User symbols
...
describe("prepareRename", () => {
  ...
  it("returns null for a system symbol", () => {
    ...
    const indexWithSysRef: CompilerIndex = {
      ...renameIndex,
      references: [
        ...renameIndex.references!,
        { sym: "nothing", type: "object", locs: ["0:1:0"] },
      ],
    };
...
describe("computeRename", () => {
  it("renames a routine at use-site and definition", () => {
    ...
    expect(fileEdits.some(e =>
      e.range.start.line === 3 && e.range.start.character === 2 && e.newText === "NewFunc"
    )).toBe(true);
    // use in OTHER line 2 col 2
    expect(otherEdits.some(e =>
      e.range.start.line === 1 && e.range.start.character === 2 && e.newText === "NewFunc"
    )).toBe(true);
  });
...
describe("computeRename action/Sub tandem", () => {
  it("renaming action also renames companion Sub routine", () => {
    ...
    const actionEdits = Object.values(changes).flat().filter(e => e.newText === "Grab");
    expect(actionEdits.length).toBeGreaterThan(0);
    // Companion FoozleSub → GrabSub
    const subEdits = Object.values(changes).flat().filter(e => e.newText === "GrabSub");
  ...
  it("renaming Sub to a name without 'Sub' suffix skips companion action", () => {
    ...
    const changes = edit!.changes!;
    // FoozleSub → Handler
    const subEdits = Object.values(changes).flat().filter(e => e.newText === "Handler");
    expect(subEdits.length).toBeGreaterThan(0);
    // Action Foozle should NOT be renamed
    const actionEdits = Object.values(changes).flat().filter(e => e.newText === "Foozle" || e.newText === "Fooz");
  ...
  it("non-action routine ending in Sub without a matching action is not treated as Sub", () => {
    ...
    if (edit) {
      const allNewTexts = Object.values(edit.changes!).flat().map(e => e.newText);
